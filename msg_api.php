...
try {
    ...
    switch ($action) {
        ...
        // All the ownership/status verification (does this appointment belong
        // to the logged-in user, and is it approved) now lives inside
        // Chat::getOrCreateChatRoom() itself, against the real "appointments" table.
        case 'get_or_create_room':
            ...
            try {
                $chatRoom = $chat->getOrCreateChatRoom($appointmentId, $userId, $userType);
            } catch (Exception $e) {
        ...
        // Get every contact (doctor or patient) this user has an approved
        // appointment with, whether or not a chat room already exists.
        // Powers the contact list on the chat page.
        case 'get_contacts':
            $contacts = $chat->getApprovedContacts($userId, $userType);
